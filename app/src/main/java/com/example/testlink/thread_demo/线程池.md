...
```
processWorkerExit()方法的调用意味着work对象(这条线程)要结束了。先看俩这个方法的俩参数。w与completedAbruptly。w是
Work对象，completedAbruptly是bool值，默认为true。只有当线程完整地执行完任务流程后才会被赋值为false。当
异常产生(completedAbruptly为true)首先是将线程池内的工作线程清理。随后将work从workers中移除。
