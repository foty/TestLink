...
##### 前缀和
通常使用于数组方面，假如有一个数组a，它的前缀和新数组为n，那么有  
n[0] = a[0];   
a[i] = n[i-1]+a[i]。
...
```
此外可能还有二分的变种题型，区别就在于边界处理不同。例如可能不能取=，或者取>=,而不能是<=。或者反过来或者边界=mid 还是=mid+1。具体问题具体分析。    
典型题目  
* 162、寻找峰值
...
动态规划问题的一般形式就是求最值。比如说求最长递增子序列，最小距离等等。这也是判断能否使用动态规划解决的因素之一。求解动态规划的核心问题就是穷举。相对的
...
##### 字典树(前缀树) <p1707:与数组中元素的最大异或值>
是一种用于快速查询某个字符串或者字符是否存在的数据结构。其核心是使用边来代表有无字符，使用节点来记录字符。可以有二维数组实现或者树形结构实现。比较普遍的使用树形
结构。核心思想是空间换时间     
字典树的几个重要点：
* 字典树的节点最少包含指向子节点的指针数组，一般是一个固定长度的数组；表示是否为某个字符串结尾字符的属性，一般是boolean值。
* 插入时，若当前子节点存在，沿着指针移动到子节点位置，继续处理下一个字符；反之，创建一个新的子节点，记录在子数组的对应位置上，然后沿着指针移动到子节点；
* 查找时，若当前子节点存在，沿着指针移动到子节点位置，继续搜索下一个字符；反之，说明字典树中不包含该前缀，返回。如此直到查找字符串的末尾，说明字典树存在该字符
串的前缀(注意是前缀而已，不是存在该字符串)；如果末尾节点的boolean值也为true，才表示字典树存在该字符串。

通常来说，题目更多会是以纯大写字母、或纯小写字母、或一定数量的字符作为子节点指针数组。所以在创建节点类时经常碰到这种格式：
```
class TrieNode {
    /**
     * 是否为某字符串的结尾字符
...
##### 优先队列(大、小根堆实现) PriorityQueue
其实是一种排序方法，由堆实现。根据根节点与子节点大小关系分为大根堆，小根堆。父节点最大则为大根堆，反正为小根堆。相同的，每次poll出来的元素必定是堆中最大
或最小的在java中有实现类：`PriorityQueue`,默认实现是一个小根堆形式，可以手动改变排序规则，实现成大根堆。
如：`PriorityQueue p = new PriorityQueue<>((o1, o2) -> o2 - o1);`
...
##### 数学思想
主要是套用一些数学公式完成解答的题目。经典的如排列组合公式、等差(比)数列公式等等    
1、 排列组合思想：从这m个数中选出2个数的组合有： A(m/2) = m·(m − 1)  
2、 分解质因数(计算模板)  
```
int ans = 0;
for (int i = 2; i * i <= n; ++i) {
